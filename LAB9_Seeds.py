...
# Cargar dataset Seeds desde archivo CSV
def load_seeds_dataset():
    url = "https://archive.ics.uci.edu/ml/machine-learning-databases/00236/seeds_dataset.txt"
    column_names = [
        "Area", "Perimeter", "Compactness", "LengthKernel", "WidthKernel", 
        "AsymmetryCoefficient", "LengthKernelGroove", "Class"
    ]
    data = pd.read_csv(url, header=None, delim_whitespace=True, names=column_names)
    X = data.iloc[:, :-1].values
    y = data.iloc[:, -1].values - 1  # Convertir clases a 0, 1, 2
    return X, y
...
# Main para ejecutar con el dataset Seeds
...
# Resultados
print("Resultados del Clasificador Euclidiano (Seeds):")
...
print("\nResultados del Clasificador 1NN (Seeds):")
